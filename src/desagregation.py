...
def desagregation(archive):
	...
	data = {}
	hours = [7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18]

	for i in range(1, MEmisions.shape[0]): 
		key = int(float(MEmisions[i][colID]))
		
		if data.get(key) is None: 
			data[key] = {'General': {'COL': [], 'ROW': [], 'LAT': [], 'LON': [], 'FID_Grilla': [], 'PM25':[], 'PM10': []}, 'PM25':{}, 'PM10':{}}
		...
		for Type in Types:
			if Type != 'General':
				for hour in range(0, 24): 
					...
					if Type == 'PM25':
						if hour in hours:
							data[key][Type][hour].append(data[key]['General']['PM25'][0]/len(hours))
						else:
							data[key][Type][hour].append(0)

					if Type == 'PM10':
						if hour in hours:
							data[key][Type][hour].append(data[key]['General']['PM10'][0]/len(hours))
						else:
							data[key][Type][hour].append(0)
